...
jQuery('document').ready(function()
{
	...
	jQuery('.shslider_section').on('click','.shs-iframe-overlay',function(){ var Iframe = jQuery(this).next('.shs-fixed-overlay').find('iframe');	 var src = Iframe.attr('src'); Iframe.attr('src', ''); Iframe.attr('src', src); jQuery(this).next('.shs-fixed-overlay').fadeIn('slow',function(){Iframe.css({'opacity':1});}); }); 	
